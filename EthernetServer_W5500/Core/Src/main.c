...
#define LISTEN_ERR_MSGs		 "LISTEN succses!\r\n"
#define SOC_CON						 "SOC CON ok!\r\n"
#define SOC_LISTEN						 "SOC Listen ok!\r\n"
#define While_Listen					 "While Listen ok!\r\n"
#define NLISTEN_ERR_MSGs		 "NOT LISTEN succses!\r\n"
#define NSOC_CON						 "NOT SOC CON ok!\r\n"
#define NSOC_LISTEN						 "NOT SOC Listen ok!\r\n"
#define NWhile_Listen					 "NOT While Listen ok!\r\n"
#define Status_ERROR_S			 	"Status Errorr!\r\n"
#define evry_ok			 	"ok evry!\r\n"
...
reconnect:
  ...
  if((retVal = socket(0, Sn_MR_TCP, 80, 0)) == 0) {
		HAL_UART_Transmit(&huart2, (uint8_t*)LISTEN_ERR_MSGs, strlen(LISTEN_ERR_MSGs), 100);
	  /* Put socket in LISTEN mode. This means we are creating a TCP server */
	  if((retVal = listen(0)) == SOCK_OK) {
			HAL_UART_Transmit(&huart2, (uint8_t*)SOC_CON, strlen(SOC_CON), 100);
		  /* While socket is in LISTEN mode we wait for a remote connection */
		  while(sockStatus = getSn_SR(0) == SOCK_LISTEN) {
			HAL_UART_Transmit(&huart2, (uint8_t*)While_Listen, strlen(While_Listen), 100);
			HAL_Delay(500);
		  ...
		  while(1) {					
			  ...
			  if(sockStatus = getSn_SR(0) == SOCK_ESTABLISHED) {
						...
						HAL_UART_Transmit(&huart2, (uint8_t*)evry_ok, strlen(evry_ok), 100);
				  uint8_t remoteIP[4];
				  uint16_t remotePort;
				  ...
				  break;
					...
					HAL_UART_Transmit(&huart2, (uint8_t*)Status_ERROR_S, strlen(Status_ERROR_S), 100);
		...
		HAL_UART_Transmit(&huart2, (uint8_t*)NSOC_CON, strlen(NSOC_CON), 100);
  } else { /* Can't open the socket. This means something is wrong with W5100 configuration: maybe SPI issue? */
	  sprintf(msg, WRONG_RETVAL_MSG, retVal);
		HAL_UART_Transmit(&huart2, (uint8_t*)NLISTEN_ERR_MSGs, strlen(NLISTEN_ERR_MSGs), 100);
	  PRINT_STR(msg);
  }
